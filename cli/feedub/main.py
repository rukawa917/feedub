...
@app.command("run")
def run(
    no_open: bool = typer.Option(False, "--no-open", help="Skip opening the browser after startup."),
) -> None:
    """Start all services."""
...
@app.command("stop")
def stop(
    remove_data: bool = typer.Option(False, "--remove-data", help="Also remove all local data and backups (DESTRUCTIVE)."),
) -> None:
    """Stop all services."""
...
def logs(
    ...
    no_follow: bool = typer.Option(False, "--no-follow", help="Print logs and exit instead of following."),
) -> None:
